...
Antworte nur mit dem Text, keine Erklärung.`
  ...
  async function ensureDraft(): Promise<string | null> {
    ...
    setSaving(true)
    const { data } = await supabase.from('newsletters').insert({ profile_id: profile.id, subject, blocks, status: 'draft' }).select().single()
    setSaving(false)
  ...
  async function sendNewsletter() {
    ...
    const res = await fetch('/api/newsletter/send', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ newsletter_id: id }) })
  ...
  return (
    <div className="min-h-screen bg-[#f7f5f2]">
      <header className="border-b border-[#e8e4de] bg-[#f7f5f2] sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <HalmIcon size={16} />
            <span className="text-xs uppercase tracking-widest text-[#9a9690] hidden sm:block">Newsletter</span>
          </div>
          <div className="flex gap-2 md:gap-3">
            {/* Mobile: Vorschau-Toggle */}
            ...
            <button onClick={saveDraft} disabled={saving || sending} className="text-xs uppercase tracking-widest text-[#9a9690] border border-[#dedad5] px-3 md:px-4 py-2 hover:border-[#2d6a2d] hover:text-[#2d6a2d] transition-colors disabled:opacity-50">
              {saving ? '…' : 'Entwurf'}
            </button>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 md:gap-10">
        ...
        {/* Editor — auf Mobile ausgeblendet wenn Vorschau aktiv */}
        ...
        {/* Vorschau — auf Mobile eingeblendet wenn Toggle aktiv */}
...
export function BlockEditor({ block, idx, total, works, claudeLoading, onUpdate, onMove, onRemove, onSuggest }: {
  block: NewsletterBlock; idx: number; total: number; works: Work[]
  ...
  return (
    <div className="border border-[#e8e4de] bg-white p-4">
      <div className="flex items-center justify-between mb-3 gap-2">
        <span className="text-xs uppercase tracking-widest text-[#9a9690] truncate">{BLOCK_LABELS[block.type]}</span>
        <div className="flex items-center gap-1 md:gap-2 flex-shrink-0">
          <button onClick={onSuggest} disabled={claudeLoading}
      ...
      {block.type === 'divider' && <p className="text-xs text-[#c5c0ba]">— Trennlinie —</p>}
      {(block.type === 'text' || block.type === 'h1' || block.type === 'h2' || block.type === 'quote') && (
        <textarea value={block.content ?? ''} onChange={e => onUpdate({ content: e.target.value })}
          placeholder={block.type === 'quote' ? 'Zitat…' : 'Text…'} rows={block.type === 'text' ? 4 : 2}
        ...
        <div className="space-y-2">
          {(block.items ?? ['']).map((item, i) => (
            <div key={i} className="flex gap-2">
              <input value={item} onChange={e => { const items = [...(block.items ?? [])]; items[i] = e.target.value; onUpdate({ items }) }}
              ...
              <button onClick={() => { const items = (block.items ?? []).filter((_, j) => j !== i); onUpdate({ items: items.length ? items : [''] }) }}
                className="text-[#9a9690] hover:text-red-400 text-sm px-1">×</button>
            </div>
          ))}
          <button onClick={() => onUpdate({ items: [...(block.items ?? []), ''] })} className="text-xs text-[#2d6a2d] hover:underline">+ Punkt hinzufügen</button>
        </div>
      )}
      {block.type === 'button' && (
        <div className="space-y-2">
          <input value={block.label ?? ''} onChange={e => onUpdate({ label: e.target.value })} placeholder="Button-Text…"
            className="w-full bg-[#faf9f7] border border-[#e8e4de] px-3 py-1.5 text-sm text-[#1a1a18] outline-none focus:border-[#2d6a2d] placeholder-[#c5c0ba]" />
          <input value={block.url ?? ''} onChange={e => onUpdate({ url: e.target.value })} placeholder="URL…"
            className="w-full bg-[#faf9f7] border border-[#e8e4de] px-3 py-1.5 text-sm text-[#1a1a18] outline-none focus:border-[#2d6a2d] placeholder-[#c5c0ba]" />
        </div>
      )}
      {block.type === 'image' && (
        <div className="space-y-2">
          <input value={block.src ?? ''} onChange={e => onUpdate({ src: e.target.value })} placeholder="Bild-URL…"
            className="w-full bg-[#faf9f7] border border-[#e8e4de] px-3 py-1.5 text-sm text-[#1a1a18] outline-none focus:border-[#2d6a2d] placeholder-[#c5c0ba]" />
          <input value={block.caption ?? ''} onChange={e => onUpdate({ caption: e.target.value })} placeholder="Bildunterschrift…"
            className="w-full bg-[#faf9f7] border border-[#e8e4de] px-3 py-1.5 text-sm text-[#1a1a18] outline-none focus:border-[#2d6a2d] placeholder-[#c5c0ba]" />
        </div>
      )}
      {block.type === 'work' && (
        <select value={block.work_id ?? ''} onChange={e => onUpdate({ work_id: e.target.value })}
          ...
          {works.map(w => <option key={w.id} value={w.id}>{w.title ?? 'Ohne Titel'} ({w.year ?? '—'})</option>)}
        </select>
      )}
      {block.type === 'video' && (
        <input value={block.video_url ?? ''} onChange={e => onUpdate({ video_url: e.target.value })} placeholder="YouTube oder Vimeo URL…"
          className="w-full bg-[#faf9f7] border border-[#e8e4de] px-3 py-1.5 text-sm text-[#1a1a18] outline-none focus:border-[#2d6a2d] placeholder-[#c5c0ba]" />
      )}
    </div>
  )
...
export function BlockPreview({ block, works }: { block: NewsletterBlock; works: Work[] }) {
  switch (block.type) {
    ...
    case 'button': return (
      <div className="mb-4">
        <span className="inline-block border border-[#2d6a2d] text-[#2d6a2d] text-xs uppercase tracking-widest px-4 py-2">{block.label || 'Button'}</span>
      </div>
    )
    case 'image': return (
      <div className="mb-4">
