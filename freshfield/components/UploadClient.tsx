...
export default function UploadClient({ profile, userId }: Props) {
  ...
  async function publish() {
    ...
    for (const file of files) {
      ...
      const { data: upload, error: uploadError } = await supabase.storage.from('works').upload(path, file)
      ...
      const publishedAt = isDraft ? null
        : scheduled && schedDate
          ? new Date(`${schedDate}T${schedTime || '20:00'}`).toISOString()
        ...
        <div className="flex items-center gap-3 md:gap-8">
          {[1, 2, 3].map(s => (
            <span key={s} className={`text-xs tracking-widest uppercase ${step === s ? 'text-ink' : 'text-soft'}`}>
              {s === 1 ? 'Dateien' : s === 2 ? 'Details' : 'Veröff.'}
            </span>
          ))}
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-5 md:px-6 py-12 md:py-16">
          ...
          <div>
            ...
            {files.length === 0 ? (
              <div
                ...
                onDrop={onDrop}
                className={`border-2 border-dashed rounded-none py-16 flex flex-col items-center gap-4 transition-colors cursor-pointer ${dragging ? 'border-ink bg-ink/5' : 'border-line hover:border-ink/40'}`}
                onClick={() => document.getElementById('fileIn')?.click()}
              >
                <span className="text-3xl text-soft">+</span>
                <p className="text-sm text-soft text-center">Dateien hierher ziehen<br /><span className="text-xs">oder tippen zum Auswählen</span></p>
              ...
              <div className="flex flex-col gap-3 mb-8">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center gap-3 md:gap-4 border border-line p-3 md:p-4">
                    {f.type.startsWith('image/') ? (
                      <img src={URL.createObjectURL(f)} alt="" className="w-14 h-14 md:w-16 md:h-16 object-cover flex-shrink-0" />
                    ) : (
                      <div className="w-14 h-14 md:w-16 md:h-16 bg-ink flex items-center justify-center text-bg text-xl flex-shrink-0">
                    ...
                    <button onClick={() => setFiles(files.filter((_, j) => j !== i))} className="text-soft hover:text-ink text-lg leading-none flex-shrink-0">✕</button>
                  </div>
                ))}
            ...
            <div className="flex justify-end mt-8">
              <button onClick={() => setStep(2)} disabled={files.length === 0} className="btn-primary disabled:opacity-30">Weiter →</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <div className="flex flex-col gap-6">
              ...
              {/* Jahr + Medium: auf Mobile untereinander, ab sm nebeneinander */}
              ...
              <div>
                ...
                <div className="border border-line p-3 flex flex-wrap gap-2 cursor-text" onClick={() => document.getElementById('tagIn')?.focus()}>
                  {tags.map(t => (
                    <button key={t} onClick={() => setTags(tags.filter(x => x !== t))} className="text-xs bg-ink/7 px-2 py-0.5 flex items-center gap-1 hover:bg-ink/15">
              ...
              <div>
                ...
                <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Ein paar Sätze zum Werk."
        ...
        {step === 3 && (
          <div>
            ...
            {/* Preview */}
            <div className="border border-line p-4 md:p-6 mb-8 flex gap-4 items-start">
              {files[0] && files[0].type.startsWith('image/') ? (
                <img src={URL.createObjectURL(files[0])} alt="" className="w-20 h-20 md:w-24 md:h-24 object-cover flex-shrink-0" />
              ) : (
                <div className="w-20 h-20 md:w-24 md:h-24 bg-ink flex items-center justify-center text-bg text-2xl flex-shrink-0">
                  {files[0]?.type.startsWith('audio/') ? '♪' : '▶'}
                </div>
              )}
              <div>
                <p className="font-serif text-lg md:text-xl">{title || 'Ohne Titel'}</p>
            ...
            {/* Schedule toggle */}
            <div className="border border-line p-4 md:p-5 mb-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setScheduled(s => !s)} className={`w-10 h-5 rounded-full relative transition-colors flex-shrink-0 ${scheduled ? 'bg-ink' : 'bg-line'}`}>
            ...
            {/* Draft option */}
            <button onClick={() => setIsDraft(d => !d)} className={`w-full border p-4 md:p-5 text-left transition-all mb-8 ${isDraft ? 'border-ink' : 'border-line'}`}>
            ...
            <div className="flex justify-between">
              ...
              <button onClick={publish} disabled={uploading} className="btn-primary disabled:opacity-40">
